<template>
    <div class="default-main ba-table-box">
        ...
        <template #default  >
            <el-button  v-auth="'audit'" v-blur :disabled="baTable.table.selection!.length > 0 ? false:true" class="table-header-operate" type="success" @click="accountAuditFn(2)">
        ...
        <div class="addPurchasingManagement-dialog">
            <el-dialog title="详情" :z-index="1000" v-model="addPurchasingManagementDialog.show1" @close="DialogCloseFn(1)" center destroy-on-close draggable>
                <div class="dialog-body">
                    <!-- table列表 -->
                    <el-row>
                        <el-table :data="addPurchasingManagementDialog.transactions" style="width: 100%;max-height: 300px;overflow-y: auto;">
                        <el-table-column prop="created_at" label="交易时间" />
                        <el-table-column prop="transaction_amount" label="交易金额" />
                        <el-table-column prop="merchant_name_location" label="商户名称" />
...
import { editIs_,cardsTransactions} from '/@/api/backend/index.ts';
...
const accountAuditFn = (type:number = 0,row={}) => {

    console.log(auth('audit'));
    if(type == 1){
        addPurchasingManagementDialog.show1 = true
        addPurchasingManagementDialog.cardsId = row.accountrequestProposal.cards_id
        cardsTransactionsFn(addPurchasingManagementDialog.cardsId)
    }else{
...
const cardsTransactionsFn =  async(id:number)=>{
    ...
    let res: anyObj = await cardsTransactions(postData)
    addPurchasingManagementDialog.transactions = res.data.row
