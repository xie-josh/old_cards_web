    ...
    new baTableApi('/admin/Account/'),
    {
        ...
        column: [
            ...
            { label: t('account.dispose_status'), prop: 'dispose_status', align: 'center', render: 'customTemplate', operator: 'eq', sortable: false,comSearchRender:'select',
             replaceValue: { '0': t('account.dispose_status 0'), '1': t('account.dispose_status 1'), '2': t('account.dispose_status 2'), '3': t('account.dispose_status 3'), '4': t('account.dispose_status 4') } ,
                customTemplate: (row: TableRow, field: TableColumn, value: any, column, index: number) => {                   
                    if(value == 0){
                        return '<span style="background-color:#d9ecff;padding:4px 9px;color:#FFF;border-radius:4px">'+t('account.dispose_status 0')+'</span>';
                    }else if(value == 1){
                        return '<span style="background-color:#67c23a;padding:4px 9px;color:#FFF;border-radius:4px">'+t('account.dispose_status 1')+'</span>';
                    }else if(value == 2){
                        return '<span style="background-color:#e6a23c;padding:4px 9px;color:#FFF;border-radius:4px">'+t('account.dispose_status 2')+'</span>';
                    }else if(value == 3){
                        return '<span style="background-color:#ff5151;padding:4px 9px;color:#FFF;border-radius:4px">'+t('account.dispose_status 3')+'</span>';
                    }else if(value == 4){
                        return '<span style="background-color:#ff5151;padding:4px 9px;color:#FFF;border-radius:4px">'+t('account.dispose_status 4')+'</span>';
                    }
