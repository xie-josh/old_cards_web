<template>
    <div class="default-main ba-table-box">
        ...
        <template #default  >            
            ...
            <el-button class="table-header-operate" type="primary" @click="accountAuditFn(6,[])">
                <span class="table-header-operate-text">批量搜索</span>
            </el-button>
        </template>
        ...
        <div class="addPurchasingManagement-dialog">
            <el-dialog title="批量搜索" style="width: 600px;" :z-index="1000" v-model="addPurchasingManagementDialog.show6" @close="DialogCloseFn(2)" center destroy-on-close draggable>
                <div class="dialog-body">
                    <el-tabs v-model="activeName"  class="demo-tabs">
                        <el-tab-pane label="账户ID" name="first">
                            
                            <el-input
                                v-model="addPurchasingManagementDialog.ids"
                                style="width: 560px"
                ...
                <template #footer>
                    <div>
                        <el-button type="success" @click="confirmAddCommodityFn(8)" >搜索</el-button>
                    </div>
                </template>
            </el-dialog>
        </div>
...
const baTable = new baTableClass(
    new baTableApi('/admin/Account/'),
    {
        ...
        column: [
            ...
            { label: t('bindingCard.open_money'), prop: 'open_money', align: 'center', operator: false, sortable: false ,width:100},
...
const accountAuditFn = (type:number = 1,row: TableRow) => {
    ...
    if(type == 3){
        ...
        addPurchasingManagementDialog.show6 = true
    }
}
...
const DialogCloseFn = (type:number = 1) => {
    ...
    addPurchasingManagementDialog.show6 = false
}
...
const confirmAddCommodityFn = (type:number = 1) => {
    ...
    if(type == 3){
        ...
        let ids = addPurchasingManagementDialog.ids
        .split('\n') // 按换行符分割
        .map(item => item.trim()) // 去掉每行的首尾空白字符
        .filter(line => line)
        .join(','); // 去掉空行
        console.log(ids,baTable.table.filter!.quickSearch)
        ...
        addPurchasingManagementDialog.ids = ''
        addPurchasingManagementDialog.show6 = false
    }
    DialogCloseFn()
